...
router.get('/dashboard', auth, async (req, res) => {
  try {
    ...
    // Scope clause — either org-wide or single user
    const userScope = orgId
      ? `u.org_id = ${parseInt(orgId)}`
    ...
    const activity = await pool.query(`
      ...
      WHERE ${userScope} AND se.status = 'done' AND se.completed_at IS NOT NULL AND se.touchpoint != 'zoho_note_added'
    ...
    const topLeads = await pool.query(`
      ...
      WHERE ${userScope} AND l.icp_score IS NOT NULL
